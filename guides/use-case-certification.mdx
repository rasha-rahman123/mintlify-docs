...
When press-safe is `null`, `proto_score_v1` did not reach the press-safe threshold — not enough acoustic evidence to certify either way. Recommended actions:

1. Check `human_safe` — if that's also `"human"`, the track is probably fine but just didn't meet press-safe's strict threshold
2. Review the `risk_series` — look for sections with elevated AI probability
